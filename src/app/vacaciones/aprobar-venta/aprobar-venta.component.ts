...
import { AprobarVentaService } from './aprobar-venta.service';
...
export class AprobarVentaComponent implements OnInit {
  ...
  constructor(private aprobarVentaService: AprobarVentaService, private modalService: NgbModal) {
  ...
  listarSolicitudesPendientesVenta() {
    this.aprobarVentaService.listarSolicitudesPendientes('E003620', '1', 1).subscribe(
      resp => {
  ...
  modalShowCancelarVenta(user: any) {
    ...
    //     resp => {
    //       console.log(resp)
    //       this.detalleSolicitudUsuarioVenta = null;
    //       this.listarSolicitudesPendientesVenta();
    //     }, 
    //     error => {
    //       console.log("Error: " + error.message)
